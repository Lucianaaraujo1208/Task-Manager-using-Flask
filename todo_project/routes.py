from flask import render_template, url_for, flash, redirect, request, escape
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=escape(form.username.data), password=hashed_password)  # Escape username
        db.session.add(user)
        db.session.commit()
        flash(f'Account Created For {escape(form.username.data)}', 'success')  # Escape username
        return redirect(url_for('login'))
...
def add_task():
    form = TaskForm()
    if form.validate_on_submit():
        task = Task(content=escape(form.task_name.data), author=current_user)  # Escape task content
        db.session.add(task)
        db.session.commit()
...
def update_task(task_id):
    ...
    if form.validate_on_submit():
        if form.task_name.data != task.content:
            task.content = escape(form.task_name.data)  # Escape updated task content
            db.session.commit()
...
def account():
    ...
    if form.validate_on_submit():
        if form.username.data != current_user.username:  
            current_user.username = escape(form.username.data)  # Escape username update
            db.session.commit()
